...
class DebugInfo(BaseModel):
    """Debug info included in every tool response for inspection.

    Attributes:
        system_prompt: The system prompt sent to Claude.
        user_message: The user message sent to Claude (with CV/JD injected).
    """
...
class ProjectResult(BaseModel):
    """A single project with Version A and Version B bullets.

    Attributes:
        title: Project title from Claude's output (e.g. "ThinkRoom | Full Stack | 2024").
        version_a: List of Version A (Tasks & Process) bullets.
        version_b: List of Version B (Impact & Outcomes) bullets.
    """
...
class ProjectsResponse(BaseModel):
    ...
    Attributes:
        projects: List of projects with their rewritten bullets.
        suggestions: Optional improvement suggestions from Claude.
        debug: Prompts sent to Claude, for inspection.
    """
...
class SkillsResponse(BaseModel):
    ...
    Attributes:
        categories: Reorganized skill categories.
        recommendations: Skills to consider adding (from JD but not in CV).
        debug: Prompts sent to Claude, for inspection.
    """
...
async def rewrite_projects(request: ToolRequest) -> ProjectsResponse:
    ...
    try:
        parsed = parse_projects_response(raw_response)
    ...
    return ProjectsResponse(**parsed, debug=DebugInfo(system_prompt=system_prompt, user_message=user_message))
...
async def rewrite_skills(request: ToolRequest) -> SkillsResponse:
    ...
    return SkillsResponse(**parsed, debug=DebugInfo(system_prompt=system_prompt, user_message=user_message))
...
async def generate_profile(request: ToolRequest) -> ProfileResponse:
    ...
    return ProfileResponse(**parsed, debug=DebugInfo(system_prompt=system_prompt, user_message=user_message))
...
async def transform_achievements(request: ToolRequest) -> AchievementsResponse:
    ...
    return AchievementsResponse(**parsed, debug=DebugInfo(system_prompt=system_prompt, user_message=user_message))
